...
import { ClockIcon, LogOutIcon, RefreshCwIcon, UserXIcon, XCircleIcon } from "lucide-react";
...
export default function PendingPage() {
  ...
  const { data: membership, isFetching, refetch, isError, error } = useMyLocationMembership({
  ...
  // 404 = no Employee profile — user needs to be set up by admin first
  ...
  const iconBg = isNoEmployee || isRejected || isTerminal
      ...
      <div className="max-w-md space-y-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            {isNoEmployee
              ? "Tài khoản của bạn chưa được kích hoạt. Vui lòng liên hệ quản lý để được hỗ trợ."
              : isTerminal
