...
- Branch workspace access: Admin sees all chi nhánh **trong tổ chức (JWT `organization_id`)**; Manager sees only chi nhánh assigned by Org Admin through `LocationManager` (assigned at Manager create via `locationIds` on `POST /employees`, or later in workspace). User sees only chi nhánh where they have Active `LocationMembership` (from department at create). UI thao tác nghiệp vụ theo chi nhánh đang chọn: sidebar links use `/{orgId}/{locationId}/{role}/...`; `/{orgId}/{role}/workspace` is only a branch-selection/redirect fallback, not an all-branch workspace. Staff: admin `POST /employees` (**User** + `departmentId`; **Manager** + `locationIds`) or employee self-onboard (`POST /auth/register-employee` → `/discover` → join request → Admin `/{orgId}/admin/join-requests`). Org-less shell: `(org-less)/discover`, `(org-less)/join-request`. Register hub `/register` → `/register/org` | `/register/employee`. Do not expose standalone org staff user create outside employee workflow.
...
- Draft schedule grid is directly editable: chips drag-and-drop to another shift/day (`PATCH /schedules/{id}/assignments/{assignmentId}` via `useMoveAssignmentMutation`), click chip to delete, `+ Thêm` to assign. `SuggestionsSheet` embeds the same interactive `ScheduleGrid` for the current Draft week before a suggestion is generated (requires `status` prop from `SchedulePanel`).
- Bedrock schedule insight: Bedrock is an optional support chat over a generated weekly schedule context snapshot. It may propose Draft-schedule mutations (Delete/Move/Add) as structured action cards (`actionMode: true` on every chat call, always-on, no toggle), but Bedrock itself never mutates anything — a mutation happens only when a human clicks **Áp dụng** on a card, at which point FE resolves the card's `(shiftDefinitionId, employeeId, date)` tuples against the currently loaded `currentAssignments` to real assignment IDs and calls the existing delete/move/create-assignment endpoints. Unresolvable (stale) items render disabled with "Ca này không còn tồn tại — làm mới snapshot." Batch apply is sequential and best-effort (no rollback); FE regenerates the insight snapshot after a batch finishes. Scheduling flows must remain usable when Bedrock is unavailable.
...
| Role app        | `app/(app)/`     | `/{orgId}/{locationId}/{admin\|manager\|user}/*`, org-only `/{orgId}/admin/onboarding`, `/{orgId}/admin/join-requests` |
| Org-less employee | `app/(org-less)/` | `/discover`, `/join-request` (User, no `organization_id`) |
