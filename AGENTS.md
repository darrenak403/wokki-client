...
- Branch workspace access: Admin sees all chi nhánh **trong tổ chức (JWT `organization_id`)**; Manager sees only chi nhánh assigned by Org Admin through `LocationManager`. UI thao tác nghiệp vụ theo chi nhánh đang chọn: sidebar links use `/{orgId}/{locationId}/{role}/...`; `/{orgId}/{role}/workspace` is only a branch-selection/redirect fallback, not an all-branch workspace. Staff are created only through the employee workflow (`POST /employees`), which creates both User + Employee with `departmentId`; same-org legacy Users without Employee are linked by that workflow; BE auto-provisions Active `LocationMembership`. Do not expose a separate "Tài khoản hệ thống" create flow for org staff. There is no employee `/join` or `/pending` gate in the current org flow.
