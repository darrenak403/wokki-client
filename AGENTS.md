...
- Auto-scheduling UX must follow the business hierarchy: choose chi nhánh → phòng ban → tuần. Branch-level `LocationSchedulingPolicy` is mandatory and rendered as a typed rule list: left column explains the rule, right column is the value to fill. Locations can add/remove rules; solver-owned rules keep stable keys. Department policy is an override; employees can belong to multiple departments. Suggest UI should surface missing setup with clear actions instead of generic empty states.
