...
import type { AttendanceListParams, ClockInRequest } from "@/types/employee";

const STALE_MS = 60 * 1000;

function defaultMonthRange(): AttendanceListParams {
  const now = new Date();
...
export function useAttendanceHistoryQuery(params?: AttendanceListParams) {
...
function openRecordRange(): AttendanceListParams {
  const now = new Date();
  return {
